...
- Contact-card detail/add controls use the same presentation restriction; received card text is not deleted.
...
- Outgoing text/captions/edits reject Unicode and custom emoji. The composer picker, sticker/GIF suggestions, reaction additions and emoji-status additions are disabled.
...
URL text is sent without a preview. Photo editing remains available without its
sticker tool. Message effects, external media URLs, opaque
...
ordinary photos/videos or arbitrary document contents, rewrite received history,
or control arbitrary third-party
