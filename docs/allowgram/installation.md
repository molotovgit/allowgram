...
The 7.2.8.7 installer targets **Windows 10 version 1903 or later, or Windows 11, on x64**. Run `Allowgram-Setup-7.2.8.7-x64.exe` only after checking its accompanying validation receipt. The installer is per-user and unsigned. Windows SmartScreen or an unknown-publisher prompt may appear; verify the source of the file and its checksum before choosing to proceed. Do not disable Windows security settings.

```powershell
Get-FileHash -Algorithm SHA256 -LiteralPath .\Allowgram-Setup-7.2.8.7-x64.exe
```
