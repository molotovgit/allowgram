...
bool MoveLegacyAlphaFolder(const QString &folder, const QString &file) {
	...
	const auto now = cExeDir() + u"AllowgramForcePortable"_q;
...
bool CheckPortableVersionFolder() {
	...
	const auto portable = cExeDir() + u"AllowgramForcePortable"_q;
...
void Launcher::init() {
	...
	QApplication::setApplicationName(u"Allowgram"_q);
