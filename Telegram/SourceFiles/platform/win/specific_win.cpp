...
#ifdef OS_WIN_STORE
		return appData.absolutePath() + u"/Allowgram UWP/"_q;
#else // OS_WIN_STORE
...
void psDoFixPrevious() {
	try {
		...
		if (SUCCEEDED(userDesktopRes)) {
			userDesktopLnk = QString::fromWCharArray(userDesktopFolder) + "\\Allowgram.lnk";
		}
		if (SUCCEEDED(commonDesktopRes)) {
			commonDesktopLnk = QString::fromWCharArray(commonDesktopFolder) + "\\Allowgram.lnk";
		}
...
void AutostartToggle(bool enabled, Fn<void(bool)> done) {
	...
	const auto success = ManageAppLink(
		...
		FOLDERID_Startup,
		L"-autostart",
		L"Allowgram autorun link.\n"
		"You can disable autorun in Allowgram settings.");
	if (done) {
...
void psSendToMenu(bool send, bool silent) {
	ManageAppLink(
		...
		FOLDERID_SendTo,
		L"--",
		L"Allowgram send to link.\n"
		"You can disable send to menu item in Allowgram settings.");
}
